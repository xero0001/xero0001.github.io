...
$$y=a_0 + a_1(x-x_0) + a_2(x-x_0)^2+a_3(x-x_0)^3+\cdots = \sum_{m=0}^{\infinity} a_m(x-x_0)^m$$
...
$$y=a_0 + a_1x + a_2x^2+a_3x^3+\cdots = \sum_{m=0}^{\infinity} a_mx^m$$
...
$$y=a_0+a_1x + a_2x^2 + a_3 x^3 + \cdots = \sum_{m=0}^{\infinity}a_m x^m$$

$$y'=1 a_1 + 2 a_2 x + 3 a_3 x^2 + \cdots = \sum_{m=1}^{\infinity} m a_m x^{m-1}$$

$$y''=1 2 a_2 + 2 3 a_3 x + \cdots = \sum_{m=2}^{\infinity} (m-1) m a_m x^{m-2}$$
...
$$y''=\sum_{m=2}^{\infinity} (m-1)ma_mx^{m-2}$$

이 둘을 더해서 0이 되어야한다!

$$\sum_{m=2}^{\infinity} (m-1)ma_mx^{m-2}+\sum_{m=0}^{\infinity} a_m x^m =0 $$
...
$$\sum_{s=0}^{\infinity} (s+1)(s+2)a_{s+2}x^{s}+\sum_{s=0}^{\infinity} a_s x^s =0 $$

$$\sum_{s=0}^{\infinity} [(s+1)(s+2)a_{s+2}+a_s] x^s = 0$$
...
$$y=a_0 \sum_{m=0}{\infinity} (-1)^m \frac{x^2m}{(2m)!} + a_1 \sum_{m=0}^{\infinity} (-1)^m \frac{x^{2m+1}}{(2m+1)!}$$
...
$$y''=\sum_{m=2}^{\infinity} m (m-1) a_m x^{m-2}$$

그대로 대입하면

$$\sum_{m=2}^{\infinity} m(m-1) a_m x^{m-2} + \sum_{m=0}^{\infinity} a_m x^{m+1} = 0 $$
...
$$\sum_{s=0}^{\infinity} (s+2)(s+1) a_{s+2} x^s + \sum_{s=1}^{\infinity} a_{s-1} x^s = 0$$

그냥 아예 묶을 순 없다.

1차부터 공통이라 묶을 수 있다. 그럼 0일때가 남는데, 그것만 따로빼준다.

$$2 \cdot 1 \cdot a_2 + \sum_{s=1}^{\infinity} [(s+2)(s+1)a_{s+2} + a_{s-1}] x^s = 0$$
