...
$$U,V$$와 $$\Sigma$$ 를 동시에 풀기보단, 양변에 $$A^T=V \Sigma^T U^T$$를 곱하여:
...
이것은 $$Q \Lambda Q^T$$형태이다.  
positive definite(혹은 semidefinite)인 행렬 $$A^TA$$를 대각화함으로써 V를 찾을 수 있다.  
$$V$$의 column들은 $$A^TA$$의 eigenvector들이고, $$\sigma_i^2$$는 eigenvalue들이다.($$\sigma_i$$가 양수인 $$\lambda_i$$의 제곱근이 되도록 선택하였다.)

$$U$$를 얻기 위해서 우리는 $$AA^T$$에 대해 같은 것을 해주어야한다.
